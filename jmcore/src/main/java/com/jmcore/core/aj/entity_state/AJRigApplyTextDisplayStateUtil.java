...
import com.jmcore.core.util.display_utils.text.BackgroundUtil;
import com.jmcore.core.util.display_utils.text.OpacityUtil;
import com.jmcore.core.util.display_utils.text.SeeThroughUtil;
import com.jmcore.core.util.display_utils.text.ShadowedUtil;
import com.jmcore.core.util.display_utils.text.TextUtil;

import java.util.*;

public class AJRigApplyTextDisplayStateUtil {

    // State keys for tracking which states have been applied
    ...
    /**
     * Ensures TextDisplayInfo has an appliedStates map and marks all states as not applied.
     */
    public static void markAllStatesNotApplied(AJRigInstance.TextDisplayInfo info) {
        if (info.appliedStates == null) info.appliedStates = new HashMap<>();
        for (String key : STATE_KEYS) info.appliedStates.put(key, false);
    }

    /**
     * Marks a specific state as not applied for a text display.
     */
    public static void markStateNotApplied(AJRigInstance.TextDisplayInfo info, String key) {
        if (info.appliedStates == null) markAllStatesNotApplied(info);
        info.appliedStates.put(key, false);
    }

    /**
     * Marks a specific state as applied for a text display.
     */
    public static void markStateApplied(AJRigInstance.TextDisplayInfo info, String key) {
        if (info.appliedStates == null) markAllStatesNotApplied(info);
        info.appliedStates.put(key, true);
    }

    /**
     * Applies all non-applied states for the set of text display UUIDs in the rig.
     * Uses only display_utils for all state applications.
     */
    public static void applyTextDisplayStates(AJRigInstance rig, Set<UUID> uuids, Plugin plugin) {
        Set<String> names = RigEntityUtil.getTextDisplayNamesFromUUIDs(rig, uuids);
        for (String name : names) {
            ...
            // --- text ---
            if (!info.appliedStates.getOrDefault("text", false)) {
                TextUtil.setText(entity, info.text);
                markStateApplied(info, "text");
            }

            // --- background (mode/color) ---
            if (!info.appliedStates.getOrDefault("background", false)) {
                if ("default".equalsIgnoreCase(info.backgroundMode)) {
                    BackgroundUtil.setBackground(entity, "default", null);
                } else {
                    BackgroundUtil.setBackground(entity, "custom", info.backgroundColor);
                }
                markStateApplied(info, "background");
            }

            // --- opacity ---
            if (!info.appliedStates.getOrDefault("opacity", false)) {
                OpacityUtil.setTextOpacity(entity, info.opacity);
                markStateApplied(info, "opacity");
            }

            // --- see_through ---
            if (!info.appliedStates.getOrDefault("see_through", false)) {
                SeeThroughUtil.setSeeThrough(entity, info.seeThrough);
                markStateApplied(info, "see_through");
            }

            // --- shadowed ---
            if (!info.appliedStates.getOrDefault("shadowed", false)) {
                ShadowedUtil.setShadowed(entity, info.shadowed);
                markStateApplied(info, "shadowed");
            }
        }
    }
}