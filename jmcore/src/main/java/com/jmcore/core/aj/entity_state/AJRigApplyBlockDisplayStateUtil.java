...
public class AJRigApplyBlockDisplayStateUtil {
    ...
    public static void markAllStatesNotApplied(AJRigInstance.BlockDisplayInfo info) {
        if (info.appliedStates == null) info.appliedStates = new HashMap<>();
        for (String key : STATE_KEYS) info.appliedStates.put(key, false);
    }

    public static void markStateNotApplied(AJRigInstance.BlockDisplayInfo info, String key) {
        if (info.appliedStates == null) markAllStatesNotApplied(info);
        info.appliedStates.put(key, false);
    }

    public static void markStateApplied(AJRigInstance.BlockDisplayInfo info, String key) {
        if (info.appliedStates == null) markAllStatesNotApplied(info);
        info.appliedStates.put(key, true);
    }

    public static void applyBlockDisplayStates(AJRigInstance rig, Set<UUID> uuids, Plugin plugin) {
        Set<String> names = RigEntityUtil.getBlockDisplayNamesFromUUIDs(rig, uuids);
        for (String name : names) {
            ...
            // --- block ---
            if (!info.appliedStates.getOrDefault("block", false)) {
                BlockUtil.setBlock(entity, info.block);
                markStateApplied(info, "block");
            }
        }
    }
}